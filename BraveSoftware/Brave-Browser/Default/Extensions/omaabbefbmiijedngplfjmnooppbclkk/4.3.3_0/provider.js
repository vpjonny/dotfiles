...
/* Protocol types */
const protocolVersionSchema = numberType();
const rawAddressSchema = stringType().regex(/^(0|-1):[0-9a-fA-F]{64}$/);
const tonConnectNetworkSchema = unionType([
    literalType(-239),
    literalType(-3),
    literalType('-239'),
    literalType('-3')
...
objectType({
    ...
    name: stringType(),
    iconUrl: stringType(),
    termsOfUseUrl: stringType().optional(),
    privacyPolicyUrl: stringType().optional()
});
...
const TonConnectTransactionPayloadMessageSchema = objectType({
    address: stringType(),
    amount: unionType([stringType(), numberType()]),
    payload: stringType().optional(),
    stateInit: stringType().optional(),
    extra_currency: recordType(stringType()).optional()
});
const batteryMessagesVariantSchema = objectType({
    messages: arrayType(TonConnectTransactionPayloadMessageSchema)
});
const gaslessMessagesVariantSchema = objectType({
    messages: arrayType(TonConnectTransactionPayloadMessageSchema),
    options: objectType({
        asset: stringType()
    })
});
const transactionRequestPayloadSchema = objectType({
    network: tonConnectNetworkSchema.optional(),
    from: rawAddressSchema.optional(),
    valid_until: numberType(),
    messages: arrayType(TonConnectTransactionPayloadMessageSchema),
    messagesVariants: objectType({
...
objectType({
    address: stringType(),
    network: stringType() // '-239' for the mainnet and '-3' for the testnet
});
...
const tonAddressItemReplySchema = objectType({
    name: literalType('ton_addr'),
    address: stringType(),
    network: stringType(),
    walletStateInit: stringType(),
    publicKey: stringType().optional()
});
const tonProofItemReplyErrorSchema = objectType({
    name: literalType('ton_proof'),
    error: objectType({
        code: nativeEnumType(CONNECT_ITEM_ERROR_CODES),
        message: stringType().optional()
    })
});
const tonProofItemReplySuccessSchema = objectType({
    name: literalType('ton_proof'),
    proof: objectType({
        timestamp: numberType(),
        domain: objectType({
            lengthBytes: numberType(),
            value: stringType()
        }),
        payload: stringType(),
...
const tonProofItemReplySchema = unionType([
    tonProofItemReplySuccessSchema,
    tonProofItemReplyErrorSchema
]);
const connectItemReplySchema = unionType([tonAddressItemReplySchema, tonProofItemReplySchema]);
...
const disconnectEventSchema = objectType({
    event: literalType('disconnect'),
    id: unionType([numberType(), stringType()]),
    payload: objectType({})
});
const disconnectRpcRequestSchema = objectType({
    method: literalType('disconnect'),
    params: tupleType([]),
    id: stringType()
});
const disconnectRpcResponseErrorSchema = objectType({
    id: stringType(),
    error: objectType({
        code: nativeEnumType(DISCONNECT_ERROR_CODES),
        message: stringType(),
        data: unknownType().optional()
    })
});
const disconnectRpcResponseSuccessSchema = objectType({
    id: stringType(),
    result: objectType({})
});
unionType([
...
const sendTransactionFeatureDeprecatedSchema = literalType('SendTransaction');
const sendTransactionFeatureSchema = objectType({
    name: literalType('SendTransaction'),
    maxMessages: numberType(),
    extraCurrencySupported: booleanType().optional()
});
const signDataTypeSchema = arrayType(enumType(['text', 'binary', 'cell']));
const signDataFeatureSchema = objectType({
    name: literalType('SignData'),
    types: signDataTypeSchema
});
const featureSchema = unionType([
    sendTransactionFeatureDeprecatedSchema,
    sendTransactionFeatureSchema,
    signDataFeatureSchema
]);
const deviceInfoSchema = objectType({
    platform: enumType(['iphone', 'ipad', 'android', 'windows', 'mac', 'linux', 'browser']),
    appName: stringType(),
    appVersion: stringType(),
    maxProtocolVersion: numberType(),
    features: arrayType(featureSchema)
});
const tonConnectEventPayloadSchema = objectType({
    items: arrayType(connectItemReplySchema),
    device: deviceInfoSchema
});
const connectEventSuccessSchema = objectType({
    event: literalType('connect'),
    id: numberType(),
    payload: tonConnectEventPayloadSchema
});
const connectEventErrorSchema = objectType({
    event: literalType('connect_error'),
    id: numberType(),
    payload: objectType({
        code: nativeEnumType(CONNECT_EVENT_ERROR_CODES),
        message: stringType()
    })
});
const connectEventSchema = unionType([connectEventSuccessSchema, connectEventErrorSchema]);
const tonAddressItemSchema = objectType({
    name: literalType('ton_addr')
});
const tonProofItemSchema = objectType({
    name: literalType('ton_proof'),
    payload: stringType()
});
const connectItemSchema = unionType([tonAddressItemSchema, tonProofItemSchema]);
const connectRequestSchema = objectType({
    manifestUrl: stringType(),
    items: arrayType(connectItemSchema)
});
objectType({
    publicKey: stringType(),
    secretKey: stringType()
});
enumType(['disconnect', 'sendTransaction', 'signData']);
...
const sendTransactionRpcRequestSchema = objectType({
    method: literalType('sendTransaction'),
    params: tupleType([stringType()]),
    id: stringType()
});
const sendTransactionRpcResponseErrorSchema = objectType({
    error: objectType({
        code: nativeEnumType(SEND_TRANSACTION_ERROR_CODES),
        message: stringType(),
        data: unknownType().optional()
    }),
    id: stringType()
});
const sendTransactionRpcResponseSuccessSchema = objectType({
    result: stringType(),
    id: stringType()
});
...
const signDataRpcRequestSchema = objectType({
    id: stringType(),
    method: literalType('signData'),
    params: tupleType([stringType()])
});
const signDataRequestPayloadBinarySchema = objectType({
    network: tonConnectNetworkSchema.optional(),
    from: rawAddressSchema.optional(),
    type: literalType('binary'),
    bytes: stringType() // base64 string
});
const signDataRequestPayloadCellSchema = objectType({
    network: tonConnectNetworkSchema.optional(),
    from: rawAddressSchema.optional(),
    type: literalType('cell'),
    schema: stringType(),
    cell: stringType() // base64 string
});
const signDataRequestPayloadTextSchema = objectType({
    network: tonConnectNetworkSchema.optional(),
    from: rawAddressSchema.optional(),
    type: literalType('text'),
    text: stringType()
});
const sendRequestPayloadSchema = unionType([
...
const signDataRequestPayloadSchema = unionType([
    signDataRequestPayloadTextSchema,
    signDataRequestPayloadBinarySchema,
    signDataRequestPayloadCellSchema
]);
...
const signDataRpcResponseErrorSchema = objectType({
    error: objectType({
        code: nativeEnumType(SIGN_DATA_ERROR_CODES),
        message: stringType()
    }),
    id: stringType()
});
const SignDataResponseSchema = objectType({
    signature: stringType(),
    address: stringType(),
    timestamp: numberType(),
    domain: stringType(),
...
const signDataRpcResponseSuccessSchema = objectType({
    id: stringType(),
...
const walletEventSchema = unionType([connectEventSchema, disconnectEventSchema]);
const walletResponseSchema = unionType([
...
unionType([walletEventSchema, walletResponseSchema]);
const appRequestSchema = unionType([
...
const tonConnectProtocolVersion = 2;
...
class ExtensionTonConnectInjectedBridge {
  constructor(provider) {
    this.provider = provider;
    this.#callbacks = [];
    this.walletInfo = tonConnectTonkeeperWalletInfo;
    this.deviceInfo = getDeviceInfo("browser", packageJson.version, 255, tonConnectTonkeeperAppName);
    this.protocolVersion = tonConnectProtocolVersion;
    this.isWalletBrowser = false;
    this.connect = async (_protocolVersion, _message) => {
      const { data: protocolVersion } = protocolVersionSchema.safeParse(_protocolVersion);
      if (protocolVersion === void 0 || protocolVersion > this.protocolVersion) {
        return this.notify(
  ...
  async send(message) {
    try {
      ...
      const result = await this.provider.send(`tonConnect_${method}`, params);
      return {
        result,
        id: String(id)
      };
...
class TonLinksInterceptor {
  constructor(provider) {
    this.provider = provider;
    this.shouldInterceptLink = (href) => {
      return Boolean(
    ...
    this.windowOpenOverride = (url, ...args) => {
  ...
  stopInterceptLinks() {
    ...
    if (this.originalWindowOpen) {
      ...
      this.originalWindowOpen = void 0;
    }
  }
}
